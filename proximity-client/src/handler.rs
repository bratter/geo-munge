...
use protocol::{ContentMode, ContentType, Request, Response};
use proximity_ipc::channel::RequestSender;

use crate::args::ClientCommand;

use super::{controllers, tracker::Tracker};
    ...
    pub fn handle(self, req: ClientCommand) -> Result<()> {
        ...
        let response = match req {
            ...
            ClientCommand::Reset(reset_args) => {
                controllers::reset(&cmd, &cmd.make_print_handler(), reset_args)
            }
            ClientCommand::Load { file: input_file } => {
                controllers::load(&cmd, &cmd.make_print_handler(), input_file)
            }
            ClientCommand::Get(get_args) => controllers::get(
                &cmd,
                &cmd.make_print_or_file_handler(&get_args.output)?,
                get_args,
            ),
            ClientCommand::Delete(delete_args) => {
                controllers::delete(&cmd, &cmd.make_print_handler(), delete_args)
            }
            ClientCommand::Knn(knn_args) => controllers::knn(
                &cmd,
                &cmd.make_print_or_file_handler(&knn_args.output)?,
                knn_args,
            ),
            ClientCommand::Window(window_args) => controllers::window(
                &cmd,
                &cmd.make_print_or_file_handler(&window_args.output)?,
                window_args,
            ),
            ClientCommand::Repl(repl_args) => controllers::repl(Arc::clone(&cmd), repl_args),
            ClientCommand::Bench(bench_args) => controllers::bench(Arc::clone(&cmd), bench_args),
        };
...
pub type Res = Arc<Mutex<ResponseHandler>>;
