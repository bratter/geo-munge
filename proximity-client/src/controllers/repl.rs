...
use super::{CommandHandler, OutputFormat, OutputOptions, ResponseHandler};
        ...
        let command_result = match select_command()? {
            ...
            Some(1) => {
                if let Some(reset_args) = build_reset() {
                    controllers::reset(&handler, &res, reset_args)
                } else {
                    Ok(())
                }
            }
            ...
            Some(2) => {
                ...
                if let Some(path) = build_path() {
                    ...
                    join_handle = Some(std::thread::spawn(move || {
                        let handle_result = controllers::load(&th, &r, Some(path));
            ...
            Some(5) => {
                ...
                if let Some(knn_args) = build_knn(&mut settings) {
                    ...
                    waiting.store(true, Ordering::Release);
                    join_handle = Some(std::thread::spawn(move || {
                        let handle_result = controllers::knn(&th, &r, knn_args);
                        w.store(false, Ordering::Release);
